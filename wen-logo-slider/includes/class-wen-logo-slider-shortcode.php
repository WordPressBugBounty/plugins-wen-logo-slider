...
?>
    ...
    <?php if (! empty($slides)): ?>
      ...
      <div class="wls-wrap">
        ...
        <?php if (isset($slider_settings['show_title']) && $slider_settings['show_title'] === '1' && isset($slider_settings['heading_size'])): ?>
          <<?php echo esc_attr($slider_settings['heading_size']); ?> class="wen-logo-slider-title"><?php echo esc_html(get_post(esc_attr($atts['id']))->post_title); ?></<?php echo esc_attr($slider_settings['heading_size']); ?>>
        <?php endif; ?>
        <?php do_action('wen_logo_slider_before_slides', $atts['id']); ?>
        <div id="<?php echo esc_attr($slider_settings['random_id']); ?>" class="<?php echo esc_attr(apply_filters('wen_logo_slider_custom_class', '', $atts['id'])); ?>" <?php if (isset($slider_settings['direction'])) {
                                                                                                                                                                            echo ($slider_settings['direction'] == 1) ? 'dir="rtl"' : "";
                                                                                                                                                                          } ?>>
  ...
  function get_slider_script($args, $settings, $image_url_list)
  {
    ...
    ?>
    <style type="text/css">
      <?php if (isset($settings['enable_navigation_arrow']) && $settings['enable_navigation_arrow'] === '1'): ?>.wls-wrap {
        padding: 0 15px
      }
    ...
    <script type="text/javascript">
      jQuery(document).ready(function($) {
        ...
        logoSlider.slick({
          ...
          autoplay: <?php if (isset($settings['auto_play'])) echo ($settings['auto_play'] === '1') ? 'true' : 'false';
                    else echo 'true' ?>,
        ...
        <?php if (isset($settings['caption']) && $settings['caption'] != "No caption"): ?>
          ...
          <?php if (isset($settings['caption_effect']) &&  $settings['caption_effect'] == "Slide Toggle"): ?>
            $('#' + randomId + ' div.panel').hover(
              function() {
                $('.ws-caption', this).slideDown('slow');
              },
              function() {
                $('.ws-caption', this).slideUp('slow');
              }
            );
          <?php else: ?>
            $('#' + randomId + ' .slick-track  div.panel').hover(
              function() {
                $('.ws-caption', this).fadeIn('slow');
              },
              function() {
                $('.ws-caption', this).fadeOut('slow');
              }
            );
          <?php endif; ?>
        <?php endif; ?>
